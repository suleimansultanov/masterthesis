...
This bibliography is organized around the four themes of the literature review. Each entry records authors, year, title, venue, a short annotation, and a relevance note tying the work to the thesis and its research gap. Where a detail could not be fully confirmed from the source page it is marked **(unverified)**.

The thesis targets a specific gap: most kernel-combination methods (classical Multiple Kernel Learning in particular) learn a single, *global* set of kernel weights shared by every sample, and they do not couple kernel selection to per-feature, noise-aware weighting. The thesis proposes *adaptive* (per-sample / per-feature) dynamic weighting that remains robust under the sensor noise typical of IoT healthcare streams. The entries below are selected to motivate, contextualize, and sharpen that gap.
...
### Akkur, E., et al. (2024). *Enhancing Cancerous Gene Selection and Classification for High-Dimensional Microarray Data Using a Novel Hybrid Filter and Differential Evolutionary Feature Selection.* Cancers / MDPI, 16(23), 3913. **(author list unverified)**
...
### Wang, T., et al. (2022). *Localized multiple kernel learning using graph modularity.* Pattern Recognition Letters, 156. **(author list unverified)**
...
### Briscik, M., et al. (2024). *Supervised Multiple Kernel Learning approaches for multi-omics data integration.* BioData Mining, 17. (also arXiv:2403.18355) **(volume unverified)**
...
### Ghanizadeh, A. N., Ghiasi-Shirazi, K., Monsefi, R., et al. (2024). *Neural Generalization of Multiple Kernel Learning.* Neural Processing Letters, 56, 12.
...
### Anand, P., et al. (2026). *A robust and lightweight support vector machine for imbalanced and noisy data via Benders decomposition.* Neurocomputing. ScienceDirect (in press). **(author list and year unverified; listed online 2025/2026)**
...
### Saif, S., et al. (2024). *Implementation of machine learning techniques with big data and IoT to create effective prediction models for health informatics.* Biomedical Signal Processing and Control, 91. ScienceDirect. **(author list unverified)**
...
### Singh, A. K., et al. (2023). *A Hybrid Particle Swarm Optimization Algorithm with Dynamic Adjustment of Inertia Weight Based on a New Feature Selection Method to Optimize SVM Parameters (DWPSO-SVM).* (PMC10047894). **(author list and venue unverified)**
...
### Tian, Y., et al. (2024). *Multimodal classification of Alzheimer's disease and mild cognitive impairment using custom MKSCDDL kernel over CNN.* (PMC10799876). **(author list unverified)**
